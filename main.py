...
if __name__ == '__main__':
    ...
    if(response == 1):
         for size in sizes:
            ...
            tf = time.time()
            insertion_time.append(round((tf - ts)*1000))

            ts = time.time()
            selection_sort(selection_array)
            tf = time.time()
            selection_time.append(round((tf - ts)*1000))

            ts = time.time()
            quick_sort(quick_array, 0, len(quick_array) - 1)
            tf = time.time()
            quick_time.append(round((tf - ts)*1000))

            ts = time.time()
            merge_sort(merge_array, 0, len(merge_array) - 1)
            tf = time.time()
            merge_time.append(round((tf - ts)*1000))

         print('selection', selection_time, 'in millisecs')
         print('insertion', insertion_time, 'in millisecs')
         print('merge', merge_time, 'in millisecs')
         print('quick', quick_time, 'in millisecs')

         fig = plt.figure(figsize=(12, 11))
         grid = plt.GridSpec(2, 4, wspace=0.6, hspace=0.8)
         ...
         plt.ylabel('Running time(millisecs)')
